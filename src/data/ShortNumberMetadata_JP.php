...
class ShortNumberMetadata_JP extends PhoneMetadata
{
    ...
    public function __construct()
    {
        $this->generalDesc = (new PhoneNumberDesc())
            ->setNationalNumberPattern('[01]\d\d(?:\d{5}(?:\d{2})?)?')
            ->setPossibleLength([3, 8, 10]);
        ...
        $this->short_code = (new PhoneNumberDesc())
            ->setNationalNumberPattern('000[259]\d{6}|1(?:0[24]|1[089]|44|89)|000[2569]\d{4}')
            ->setExampleNumber('102');
        $this->standard_rate = PhoneNumberDesc::empty();
        $this->carrierSpecific = PhoneNumberDesc::empty();
        $this->smsServices = (new PhoneNumberDesc())
            ->setNationalNumberPattern('000[259]\d{6}|000[2569]\d{4}')
